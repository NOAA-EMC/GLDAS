...
! total soil moisture
   call nemsio_readrecv(gfile, "soilw", "0-10 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soilw1.", rc)
   soilm_tot(:,:,1) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soilw 1 ',maxval(soilm_tot(:,:,1)),minval(soilm_tot(:,:,1))
   call nemsio_readrecv(gfile, "soilw", "10-40 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soilw2.", rc)
   soilm_tot(:,:,2) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soilw 2 ',maxval(soilm_tot(:,:,2)),minval(soilm_tot(:,:,2))
   call nemsio_readrecv(gfile, "soilw", "40-100 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soilw3.", rc)
   soilm_tot(:,:,3) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soilw 3 ',maxval(soilm_tot(:,:,3)),minval(soilm_tot(:,:,3))
   call nemsio_readrecv(gfile, "soilw", "100-200 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soilw4.", rc)
   soilm_tot(:,:,4) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soilw 4 ',maxval(soilm_tot(:,:,4)),minval(soilm_tot(:,:,4))

! liq soil moisture
   call nemsio_readrecv(gfile, "soill", "0-10 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soill1.", rc)
   soilm_liq(:,:,1) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soill 1 ',maxval(soilm_liq(:,:,1)),minval(soilm_liq(:,:,1))
   call nemsio_readrecv(gfile, "soill", "10-40 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soill2.", rc)
   soilm_liq(:,:,2) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soill 2 ',maxval(soilm_liq(:,:,2)),minval(soilm_liq(:,:,2))
   call nemsio_readrecv(gfile, "soill", "40-100 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soill3.", rc)
   soilm_liq(:,:,3) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soill 3 ',maxval(soilm_liq(:,:,3)),minval(soilm_liq(:,:,3))
   call nemsio_readrecv(gfile, "soill", "100-200 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas soill 4.", rc)
   soilm_liq(:,:,4) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas soill 4 ',maxval(soilm_liq(:,:,4)),minval(soilm_liq(:,:,4))

! soil temp
   call nemsio_readrecv(gfile, "tmp", "0-10 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas tmp1.", rc)
   soil_temp(:,:,1) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas tmp 1 ',maxval(soil_temp(:,:,1)),minval(soil_temp(:,:,1))
   call nemsio_readrecv(gfile, "tmp", "10-40 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas tmp2.", rc)
   soil_temp(:,:,2) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas tmp 2 ',maxval(soil_temp(:,:,2)),minval(soil_temp(:,:,2))
   call nemsio_readrecv(gfile, "tmp", "40-100 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas tmp3.", rc)
   soil_temp(:,:,3) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas tmp 3 ',maxval(soil_temp(:,:,3)),minval(soil_temp(:,:,3))
   call nemsio_readrecv(gfile, "tmp", "100-200 cm down", 1, dummy1d, 0, iret=rc)
   if (rc /= 0) call error_handler("reading gldas tmp4.", rc)
   soil_temp(:,:,4) = reshape(dummy1d, (/i_gldas,j_gldas/))
   print*,'gldas tmp 4 ',maxval(soil_temp(:,:,4)),minval(soil_temp(:,:,4))
